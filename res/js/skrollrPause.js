...
/**
 * add JuSkrollr fade
 * author chuncheng.li@taobao.com
 * time 2013-05-05
 */
var JuSkrollr = function(options){
	/*
	 * Global api.
	 */
	...
	//Credits go to Erik M枚ller (http://my.opera.com/emoller/blog/2011/12/20/requestanimationframe-for-smart-er-animating)
	(function() {
	...
	function Skrollr(options) {
		...
		_instance.refresh(_container);

		skrollr.addEvent(window, 'resize', _reflow);

		//Let's go.
		_instance.run(); // changelog by chuncheng.li@taobao.com
		return _instance;
	}

	// add run methods by chuncheng.li@taobao.com
	Skrollr.prototype.run = function(){

		//This is how the cool kids use requestAnimationFrame
		//http://paulirish.com/2011/requestanimationframe-for-smart-animating/
		if(_instance.isAllStop){
			return;
		}
		requestAnimFrame(_instance.run);
	...
	// export skrollr
	Skrollr.prototype.skrollr = skrollr;

	/**
	 * add stop method. the animate will stop now and save caputer
	 * author: 崎轩(chuncheng.li@taobao.com)
	 */
	Skrollr.prototype.stop = function(){
		_instance.isAllStop = true;
	};

	/**
	 * add resume method. the animate will resume now from save caputer
	 * author: 崎轩(chuncheng.li@taobao.com)
	 */
	Skrollr.prototype.resume = function(){
		_instance.isAllStop = false;
		_instance.run();
	};
	...
	Skrollr.prototype.refresh = function(elements) {
		...
		if(elements === undefined || elements === null) {
			...
			_skrollables = [];
			_skrollableIdCounter = 0;
			elements = document.getElementById(elements).getElementsByTagName('*');
		} else {
	...
	*/
	...
	return skrollr.init(options);
}

	window.JuSkrollr = JuSkrollr;
	window.JuSkrollr.plugins = {};
}(window, document));
